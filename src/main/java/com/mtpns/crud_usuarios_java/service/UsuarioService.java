...
public class UsuarioService {
    ...
    public void adicionarUsuario(Usuario usuario){
        ...
        PreparedStatement stmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            showAlert("Operação em Memória", "Conexão indisponível. O noco usuário foi salvo apenas na memória local");
    ...
    public void sincronizarComBanco() {
        ...
        List<Usuario> usuariosParaSync = usuarios.stream().filter(u -> u.getId() < 0).collect(Collectors.toList());
        if (!usuariosParaSync.isEmpty()) {
            ...
            showAlert("Reconectado", "Conexã com o banco foi reestabelecida.");
        }
        carregarUsuarioDoBanco();
    }
